...
case "$ARCHITECTURE" in
  amd64 | aarch64)
    ARCH_ALIAS=$(echo $ARCHITECTURE | sed 's/aarch64/arm64/')
    ;;
  *)
    echo "${R}Error: Unsupported architecture. Installation denied.${W}"
    exit 1
    ;;
esac
...
# Notice
echo ${C}"Your architecture is $ARCHITECTURE . Using compatible alias: $ARCH_ALIAS"
...
# Links
URL="https://raw.githubusercontent.com/EXALAB/Anlinux-Resources/master/Rootfs/Fedora/$ARCH_ALIAS/fedora-rootfs-architecture.tar.xz"
echo ${G}"Using default URL: $URL"${W}
sleep 1
echo " "
